...
import { beginCheckoutBooking } from '@/app/actions/client-booking-actions'
...
export default async function CheckoutSummaryPage({ searchParams }: PageProps) {
  const params = await searchParams
  const serviceId = params.serviceId
  const start = params.start
  const workerId = params.workerId

  if (!serviceId || !start || !workerId) {
  ...
  if (Number.isNaN(startTime.getTime())) {
    redirect('/book/datetime?serviceId=' + encodeURIComponent(serviceId))
  }
            ...
            <div className="bg-[#f9f3ea] border border-[#8d7167] p-6 mb-8 flex flex-col md:flex-row gap-6 md:gap-12">
              ...
              <div className="flex-1 border-t md:border-t-0 md:border-l border-[#8d7167]/30 pt-4 md:pt-0 md:pl-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <p className="text-lg text-[#1d1b16] font-bold">{worker.user.name}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-10 flex-grow">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center py-3 border-b border-[#8d7167]/20 group hover:bg-[#f3ede4] px-2 -mx-2 transition-colors">
                  <div>
                    <p className="text-base text-[#1d1b16] font-bold">{service.name}</p>
                    <p className="font-mono text-xs text-[#5f5e5e]">{service.duration} mins</p>
                  </div>
                  <p className="font-mono text-base text-[#1d1b16] font-bold">{formatKes(subtotal)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-auto">
              <div className="bg-[#f3ede4] border border-[#8d7167] p-6 mb-8">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <p className="font-mono text-sm text-[#1d1b16] font-bold">{formatKes(subtotal)}</p>
                </div>
                <div className="flex justify-between items-center mb-4 pb-4 border-b border-[#8d7167]/30">
                  ...
                  <p className="font-mono text-sm text-[#1d1b16] font-bold">{formatKes(fees)}</p>
                </div>
                <div className="flex justify-between items-center mb-6">
                  <p className="text-xl text-[#1d1b16] font-bold">Total</p>
                  <p className="text-xl text-[#1d1b16] font-bold">{formatKes(total)}</p>
                </div>
                ...
                <div className="pt-4 border-t-2 border-[#8d7167] border-dashed flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    ...
                    <p className="font-mono text-xs text-[#5f5e5e] mt-1">Remaining {formatKes(remainder)} due at salon</p>
                  </div>
                  <p className="text-3xl text-[#a83a00] font-bold tracking-tight">{formatKes(deposit)}</p>
                </div>
              </div>

              <div className="flex flex-col gap-4">
                <form action={beginCheckoutBooking}>
                  <input type="hidden" name="serviceId" value={service.id} />
                  <input type="hidden" name="workerId" value={worker.id} />
                  <input type="hidden" name="start" value={startTime.toISOString()} />
                  <button
                    type="submit"
                    className="w-full bg-[#a83a00] text-white font-mono text-sm font-bold uppercase tracking-wider py-4 border border-[#8d7167] hover:bg-[#802a00] transition-colors shadow-[2px_2px_0px_#1d1b16] active:translate-y-[2px] active:shadow-none flex justify-center items-center gap-2"
                  >
                ...
                <Link
                  href={`/book/datetime?serviceId=${encodeURIComponent(service.id)}`}
                  className="md:hidden w-full flex justify-center items-center text-[#594139] hover:text-[#a83a00] transition-colors font-mono text-sm font-bold gap-2 py-4"
                >
                  <ArrowLeft size={16} />
                  Back to Date & Time
                </Link>
